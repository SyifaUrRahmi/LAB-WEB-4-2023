...
<body>
    ...
    $data_query = "SELECT nama, nim, prodi, email FROM mahasiswa WHERE email != 'admin@gmail.com'";
...
if (isset($_POST['add_submit'])) {
        ...
        if ($check_result->num_rows > 0) {
            ...
            $insert_query = "INSERT INTO mahasiswa (nama, nim, prodi, email, password) VALUES ('$nama', '$nim', '$prodi', '$email', '$hashed_password')";
    ...
    if (isset($_POST['update_submit'])) {
        ...
        $check_query = "SELECT email FROM mahasiswa WHERE email = '$email_update'";
        $check_result = $conn->query($check_query);
        
        if ($check_result->num_rows > 0) {
            $hashed_password = password_hash($password_update, PASSWORD_DEFAULT);
            
            $update_query = "UPDATE mahasiswa SET nama='$nama_update', nim='$nim_update', prodi='$prodi_update', password='$hashed_password' WHERE email='$email_update'";
    ...
    if (isset($_POST['delete_submit'])) {
        ...
        $delete_query = "DELETE FROM mahasiswa WHERE email = '$email_delete'";
